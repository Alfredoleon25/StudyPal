...
export default function ChatWindow() {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newMessage.trim()) return;
    console.log(isNewChat, pendingChat);
    try {
      if (isNewChat && pendingChat) {
        console.log("this is the newchat and pending chat",isNewChat, pendingChat);
        console.log("Sending message: creating new chat", newMessage);
        const chatResponse = await api("/chats", {
          method: "POST",
          body: JSON.stringify({
            learnerId: pendingChat.learnerId,
        ...
        console.log("Sending message: in chat created", newMessage);
        const response = await api(`/chats/${chatId}/messages`, {
          method: "POST",
          body: JSON.stringify({ senderId: user.id, content: newMessage }),
        });
